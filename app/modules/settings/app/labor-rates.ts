...
import type { LaborRate, LaborRateKind, SettingsRepository } from "../domain/settings-repository";

// Org must always have at least one active labor rate (matches prototype guard).
const MIN_ACTIVE_LABOR_RATES = 1;
const DEFAULT_LABOR_RATE_KIND: LaborRateKind = "hourly";
const LABOR_RATE_KINDS: readonly LaborRateKind[] = ["hourly", "flat_fee"];

const isLaborRateKind = (v: string): v is LaborRateKind =>
  (LABOR_RATE_KINDS as readonly string[]).includes(v);
...
export class CreateLaborRateUseCase {
  ...
  async exec(cmd: CreateLaborRateCommand, orgId: string): Promise<Result<LaborRate, AppError>> {
    ...
    if (cmd.kind !== undefined && !isLaborRateKind(cmd.kind)) {
      return err(validation("invalid kind", "kind"));
    }
    const rate = await this.repo.createLaborRate({
      ...
      kind: cmd.kind ?? DEFAULT_LABOR_RATE_KIND,
...
export class UpdateLaborRateUseCase {
  ...
  async exec(cmd: UpdateLaborRateCommand, orgId: string): Promise<Result<LaborRate, AppError>> {
    ...
    const next: LaborRate = {
      ...
      kind: cmd.kind !== undefined && isLaborRateKind(cmd.kind) ? cmd.kind : existing.kind,
