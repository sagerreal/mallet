...
export class CompleteQboConnect {
  ...
  async exec(cmd: CompleteQboConnectCommand, orgId: string): Promise<Result<QboConnection, AppError>> {
    ...
    const created = QboConnection.create({
      ...
      defaultInvoiceItemQboId: existing?.props.defaultInvoiceItemQboId ?? null,
      defaultInvoiceItemName: existing?.props.defaultInvoiceItemName ?? null,
      sendInvoices: existing?.props.sendInvoices ?? false,
