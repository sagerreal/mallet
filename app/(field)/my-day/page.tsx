...
function todayStops(jobs: Job[], techId: string): TodayStop[] {
  ...
  jobs
    ...
    .forEach((j) => {
      (j.visits ?? []).forEach((v) => {
        if (v.techId === techId && v.date === today && v.start != null) {
...
export default function MyDayPage() {
  ...
  const [myTech, setMyTech] = useState<string>("");
  useEffect(() => {
    if (!me) return;
  ...
  function callLead(leadId: string): void {
