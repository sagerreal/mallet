...
// The lifecycle of a click-to-call. Two legs: Twilio rings the AGENT first ("dialing"),
// and only once they answer does it bridge the customer ("in_progress").
...
export class OutboundCall {
  ...
  static create(props: OutboundCallProps): Result<OutboundCall, ValidationError> {
    ...
    if (props.agentNumber === props.toNumber) {
  ...
  // The provider accepted the request and is ringing the agent leg.
